...
import com.example.client_mobile.presentation.common.screens.NotificationType
import com.example.client_mobile.presentation.common.screens.AppNotification
        ...
        try {
            ...
            if (response.isSuccessful) {
                val items = response.body()?.data?.map { dto ->
                    val type = try {
                        NotificationType.valueOf(dto.type.uppercase())
                    } catch (_: Exception) {
